...
func main() {
	...
	dataMap := map[string]func(columnType gorm.ColumnType) string{
		"tinyint":   func(columnType gorm.ColumnType) string { return "int64" },
		"smallint":  func(columnType gorm.ColumnType) string { return "int64" },
		"mediumint": func(columnType gorm.ColumnType) string { return "int64" },
	...
	autoUpdateTimeField := gen.FieldGORMTag("update_time", func(tag field.GormTag) field.GormTag {
		return tag.Set("column", "update_time").
			Set("type", "int unsigned").
			Append("autoUpdateTime")
	})

	autoCreateTimeField := gen.FieldGORMTag("create_time", func(tag field.GormTag) field.GormTag {
		return tag.Set("column", "create_time").
			Set("type", "int unsigned").
			Append("autoCreateTime")
	})
	...
	// 定义与 User 的一对多关系
	userRelateConfig := &field.RelateConfig{
		GORMTag: field.GormTag{}.Set("foreignKey", "UID"),
	}
	...
	User := g.GenerateModel("user",
		append(
			...
			gen.FieldRelate(field.HasMany, "Address", Address, userRelateConfig),
		)...,
	)
	// 定义与 Address 的多对一关系
	addressRelateConfig := &field.RelateConfig{
		GORMTag: field.GormTag{}.Set("foreignKey", "UID"),
	}
	...
	Address = g.GenerateModel("address",
		append(
			fieldOpts,
			gen.FieldRelate(field.BelongsTo, "User", User, addressRelateConfig),
		)...,
	)
